...
if __name__ == '__main__':
    ...
    label = np.argmax(np.load('label.npy', allow_pickle=True), axis=1).flatten()
    x_train = np.load('x_train.npy', allow_pickle=True)

    x_train,val_train,x_label,val_label = train_test_split(x_train,label,test_size=0.1,stratify=label)
