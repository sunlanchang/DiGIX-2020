...
def gen_list_df(feature):
    try:
        train = pd.read_csv(r'/home/zhangqibot/proj/digix/zlh/slc/slc/data/train_data.csv', sep='|',
                            dtype=str, nrows=50000).drop(columns=['communication_onlinerate']).astype(int)
        train = reduce_mem(train, use_float16=True)

        test = pd.read_csv(r'/home/zhangqibot/proj/digix/zlh/slc/slc/data/test_data_A.csv', sep='|', dtype=str, nrows=50000).drop(
            columns=['id', 'communication_onlinerate']).astype(int)
        test.insert(0, 'label', np.ones([50000]))
        test['label'] = 2
        ...
        for i in tqdm(index_list):
            ...
            for j in ptd_set:
                feature_list_ = []
                buf_list = []
                buf_list = index_get_group.query(
                    'pt_d < @j')[feature].values.flatten().tolist()
                buf_list.append(2)
...
if __name__ == '__main__':
    # 'task_id','adv_id','dev_id','inter_type_cd','spread_app_id','tags','app_first_class','app_second_class','his_app_size','his_on_shelf_time','app_score',,'creat_type_cd','adv_prim_id','indu_name'
