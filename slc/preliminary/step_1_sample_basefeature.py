...
data['pt_d'] = data['pt_d'].astype(int)
...
# data1: train  test_data_A
# data2: test_data_B
data1 = data.query('pt_d<=7').drop_duplicates(
    subset=cols)  # 重复样本去掉 不清楚test_b是不是pt_d=8
# 用B榜数据
data2 = data.query('pt_d==9')

# %%


def get_sample(df, day, rate=5):
    # 对于某一天的数据，label=1全部取到, 对于label=0的数据取5倍于正样本
...
for var in sparse_features:
    ...
    print(data.loc[data[var] == -1, var].shape)
...
def cls(df):
    f = df.columns[-1]
    mapping = dict(df.query('pt_d<=7')[f].value_counts(
    ) / df.query('pt_d<=7')[f].value_counts().max())  # 只统计train
    mapping_test = dict(df.query('pt_d==9')[f].value_counts(
    ) / df.query('pt_d==9')[f].value_counts().max())  # 只统计test
...
# 只是用test_B数据
test_df = data[data["pt_d"] == 9].copy().reset_index()
train_df = data[data["pt_d"] <= 7].reset_index()
del data
gc.collect()
...
# 调整样本顺序到一致
database = Cache.reload_cache(
    f'CACHE_data_step_1_feature_0924_r{sample_rate}.pkl')
database = database[['index']]
gc.collect()
df_fe = database.merge(df_fe, on='index', how='left')  # 慢
Cache.cache_data(
    df_fe, nm_marker=f'data_step_1_feature_0924_r{sample_rate}')
