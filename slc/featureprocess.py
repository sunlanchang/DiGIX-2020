...
for i in tqdm(range(datatrain.shape[0])):
    dflist.append([int(j) if index != 32 else j for index,
                   j in enumerate(datatrain[columns_str].iloc[i].split('|'))])
...
for i in tqdm(range(datatest.shape[0])):
    dflisttst.append([int(j) if index != 32 else j for index,
                      j in enumerate(datatest[columns_str].iloc[i].split('|'))])
del datatest
gc.collect()
...
# 开源特征
##########################cate feature#######################
cate_cols = ['slot_id', 'net_type', 'task_id', 'adv_id', 'adv_prim_id', 'age', 'app_first_class',
...
feature_target = ['task_id', 'adv_id', 'dev_id',
                  'slot_id', 'spread_app_id', 'indu_name']
...
enc_list = group_list + ['net_type', 'task_id', 'adv_id', 'adv_prim_id', 'age', 'app_first_class',
                         'app_second_class', 'career', 'city', 'consume_purchase', 'uid', 'uid_count', 'dev_id', 'tags', 'slot_id']
