...
if __name__ == '__main__':
    train = pd.read_csv(r'train_data.csv', sep='|', dtype=str).drop(columns = ['communication_onlinerate']).astype(str)
    train['age_slot_id'] = train['age']+train['slot_id']
    train['city_slot_id'] = train['city']+train['slot_id']
    train['city_rank_slot_id'] = train['city_rank']+train['slot_id']
    train['device_name_slot_id'] = train['device_name']+train['slot_id']
    train['career_slot_id'] = train['career']+train['slot_id']
    train['gender_slot_id'] = train['gender']+train['slot_id']
    train['residence_slot_id'] = train['residence']+train['slot_id']
    train = train.astype(int)
    train = reduce_mem(train, use_float16=True)
    test = pd.read_csv(r'test_data_A.csv', sep='|', dtype=str).drop(columns = ['id','communication_onlinerate']).astype(str)
    test.insert(0, 'label', np.ones([1000000]))
    test['label'] = 2
    test['age_slot_id'] = test['age']+test['slot_id']
    test['city_slot_id'] = test['city']+test['slot_id']
    test['city_rank_slot_id'] = test['city_rank']+test['slot_id']
    test['device_name_slot_id'] = test['device_name']+test['slot_id']
    test['career_slot_id'] = test['career']+test['slot_id']
    test['gender_slot_id'] = test['gender']+test['slot_id']
    test['residence_slot_id'] = test['residence']+test['slot_id']
